...
- `config/defaults/sources.json` — 91 встроенных источника (21 RSS, 43 Twitter, 19 GitHub, 8 Reddit), которые вы можете добавлять/удалять/исправлять прямо через агента или руками на сервере. 
...
Multi-Parser это переработанный форк [draco-agent/tech-news-digest](https://github.com/draco-agent/tech-news-digest), который я превратил в супер-экономичный вариант, подходящий под любого юзера, где каждый может закастомить моё решение под себя или продолжить масштабировать парсер для своего AI агента.
